...
处理 crop_by_yolo_with_metadata.py 的输出目录：
    ...
    2. 识别每个 caption 图片的文字 -> 插图名，并复制对应插图重命名（去除开头可能多余的 'O' 或 'o' 或 '0'）
...
if __name__ == "__main__":
    # ===== 配置参数（根据实际路径修改）=====
    # 输入：按title分类分割好的目录
    INPUT_ROOT = r"data\Temp_data\images_cropped_villages"   
    # 输出：每个村落的 OCR 结果和插图保存目录，命名为 "序号_村名"   
    OUTPUT_ROOT = r"data\Final_output\各村OCR结果"                     
    # 临时 JSON 目录
    TEMP_JSON_DIR = r"data\Temp_data\ocr_json_results"               
